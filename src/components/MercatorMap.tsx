...
export default function MercatorMap({
  ...
  useEffect(() => {
      ...
      const addGridLines = () => {
        ...
        for (let lat = -75; lat <= 75; lat += 15) {
          ...
          if (lat !== 0) {
            L.marker([lat, 0], {
              icon: L.divIcon({
                html: `<div style="background: rgba(0,0,0,0.7); color: white; padding: 2px 6px; border-radius: 3px; font-size: 11px;">${Math.abs(
                  lat
                )}°${lat > 0 ? "N" : "S"}</div>`,
                iconSize: [40, 20],
    ...
    if (mapInstanceRef.current) {
      ...
      const zenithLatLngs: [number, number][] = zenithLine.map((point) => [
        point.lat,
        point.lng,
      ]);
      ...
      subsolarMarkerRef.current = L.marker(
        [subsolarPoint.lat, subsolarPoint.lng],
        {
          icon: sunIcon,
        }
      ).addTo(mapInstanceRef.current);
      ...
      subsolarMarkerRef.current.bindPopup(`
        <div style="text-align: center; font-family: Arial;">
          ...
          <p style="margin: 3px 0;"><strong>Latitude:</strong> ${subsolarPoint.lat.toFixed(
            2
          )}°</p>
          <p style="margin: 3px 0;"><strong>Longitude:</strong> ${subsolarPoint.lng.toFixed(
            2
          )}°</p>
      ...
      <div className="map-wrapper">
        ...
        <div className="map-legend">
          ...
          <div className="legend-item">
            <div
              className="legend-color"
              style={{ backgroundColor: "#ffff00", borderRadius: "50%" }}
            ></div>
